    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="借阅编号" prop="jieyuebianhao">
            <el-input v-model="ruleForm.jieyuebianhao" 
                placeholder="借阅编号" clearable :disabled=" false  ||ro.jieyuebianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="续借天数" prop="xujietianshu">
            <el-input v-model.number="ruleForm.xujietianshu" 
                placeholder="续借天数" clearable :disabled=" false  ||ro.xujietianshu"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`tushuxujie/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('tushuxujie/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`tushuxujie/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
